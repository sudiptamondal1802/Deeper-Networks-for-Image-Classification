...
for i in range(10):
  ...
  plt.yticks([])
#save the images
plt.savefig(SAVE_PATH + 'MNIST_GOOGLENET_images.png')
plt.close()
...
#     output, aux2, aux1 = network(data)
#     loss1 = loss_function(output, target)
#     loss2 = loss_function(aux1, target)
#     loss3 = loss_function(aux2, target)
#     loss = loss1 + 0.3 * (loss2 + loss3)
  ...
  if(best_valid_acc < valid_accuracy_epoch):
    best_valid_acc = valid_accuracy_epoch
    print('Saving best model')
    torch.save(network.state_dict(), SAVE_PATH + 'model.pth')
    torch.save(optimizer.state_dict(), SAVE_PATH + 'optimizer.pth')

end_time = time.time()
print("End time of the training:",end_time)
...
plt.ylabel('Loss')
plt.legend()
plt.savefig(SAVE_PATH + 'MNIST_GOOGLENET_loss_plot.png')
plt.close()
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig(SAVE_PATH + 'MNIST_GOOGLENET_accuracy_plot.png')
plt.close()

"""# Test the trained network"""
print("Test results for MNIST Googlenet")
...
plt.savefig(SAVE_PATH + 'MNIST_GOOGLENET_sample_test_results.png')
plt.close()
...
my_file = open(os.path.join(PICKLE_DUMP,'mnist_googlenet_model1'), 'wb') 
my_file = pickle.dump(mnist_googlenet_model1, my_file)
