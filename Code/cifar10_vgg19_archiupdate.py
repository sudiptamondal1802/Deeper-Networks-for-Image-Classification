...
# save the images
plt.savefig(SAVE_PATH + 'Cifar10_VGG19_images.png')
plt.close()
...
class VGG19(nn.Module):
    def __init__(self):
        ...
        # Add dropout after every maxpool layer
        features_list = list(self.model.features)
        new_features_list = []
        for features in features_list:
            new_features_list.append(features)
            if isinstance(features, nn.MaxPool2d):
                new_features_list.append(nn.Dropout(p=0.4, inplace=True))
            if isinstance(features, nn.ReLU):
                new_features_list.remove(features)
                new_features_list.append(mish_layer())
...
plt.ylabel('Loss')
plt.legend()
plt.savefig(SAVE_PATH + 'Cifar10_VGG19_loss_plot.png')
plt.close()
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig(SAVE_PATH + 'Cifar10_VGG19_accuracy_plot.png')
plt.close()

"""# Test the trained network"""
print("Test results for Cifar10 VGG19")
# track test loss
test_loss = 0.0
...
plt.savefig(SAVE_PATH + 'Cifar10_VGG19_model3_cm.png')
plt.show()
plt.close()
...
plt.savefig(SAVE_PATH + 'Cifar10_VGG19_sample_test_results.png')
plt.close()
...
my_file = open(os.path.join(PICKLE_DUMP,'cifar10_vgg19_model3'), 'wb') 
my_file = pickle.dump(cifar10_vgg19_model3, my_file)
